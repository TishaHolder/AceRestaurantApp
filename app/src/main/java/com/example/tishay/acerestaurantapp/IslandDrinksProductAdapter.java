 ...
 * Final Project Part I Milesone One: IslandDrinksProductAdapter.java
 * Populates the data for the island drinks products in the list view.
...
public class IslandDrinksProductAdapter extends BaseAdapter {
    ...
    //constructor
    public IslandDrinksProductAdapter(List<Product> list, LayoutInflater inflater, boolean showQuantity, boolean showPrice) {
        mProductList = list;
        mInflater = inflater;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int product_quantity = IslandDrinksShoppingCartHelper.getProductQuantity(curProduct);
        double product_price = curProduct.price;

        //if the user hasn't added any products from the island drinks category to their shopping cart, set the quantity to one (for calculation purposes)
        //if(IslandDrinksShoppingCartHelper.getProductQuantity(curProduct) == 0)
        //{
            //product_quantity = 1;
        //}
        ...
        if (mShowQuantity) {
            item.productQuantity.setText("Quantity: "
                    + IslandDrinksShoppingCartHelper.getProductQuantity(curProduct));
        } else {
            // Hid the view
