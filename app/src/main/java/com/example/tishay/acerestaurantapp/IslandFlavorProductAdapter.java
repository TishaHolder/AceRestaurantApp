 ...
 * Populates the data for the island flavor products in the list view.
...
public class IslandFlavorProductAdapter extends BaseAdapter {
    ...
    //constructor
    public IslandFlavorProductAdapter(List<Product> list, LayoutInflater inflater, boolean showQuantity, boolean showPrice) {
        mProductList = list;
        mInflater = inflater;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int product_quantity = IslandFlavorShoppingCartHelper.getProductQuantity(curProduct);
        double product_price = curProduct.price;

        //if the user hasn't added any products from the island flavor category to their shopping cart, set the quantity to one (for calculation purposes)
        //if(IslandFlavorShoppingCartHelper.getProductQuantity(curProduct) == 0)
        //{
            //product_quantity = 1;
        //}
        ...
        if (mShowQuantity) {
            item.productQuantity.setText("Quantity: "
                    + IslandFlavorShoppingCartHelper.getProductQuantity(curProduct));
        } else {
            // Hid the view
