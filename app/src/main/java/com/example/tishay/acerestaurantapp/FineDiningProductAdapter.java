...
public class FineDiningProductAdapter extends BaseAdapter
{
    ...
    private boolean mShowQuantity;//boolean value that indicates whether to show the quantity in the shopping cart
    ...
    public FineDiningProductAdapter(List<Product> list, LayoutInflater inflater, boolean showQuantity, boolean showPrice) {
        mProductList = list;
        mInflater = inflater;
    ...
    public View getView(int position, View convertView, ViewGroup parent)
    {
        ...
        item.productTitle.setText(curProduct.title);//sets the price of the current product

        int product_quantity = FineDiningShoppingCartHelper.getProductQuantity(curProduct);//stores the current product quantity
        double product_price = curProduct.price;//stores the current product price
        ...
        //if(FineDiningShoppingCartHelper.getProductQuantity(curProduct) == 0)
        //{
            //product_quantity = 1;
        //}
        ...
        if (mShowQuantity)
        {
            item.productQuantity.setText("Quantity: "
                    + FineDiningShoppingCartHelper.getProductQuantity(curProduct));
        } else {
            // Hide the view
