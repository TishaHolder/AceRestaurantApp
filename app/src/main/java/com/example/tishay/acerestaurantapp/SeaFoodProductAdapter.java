 ...
 * Final Project Part I Milesone One: SeaFoodProductAdapter.java
 * Populates the data for the sea food products in the list view.
...
public class SeaFoodProductAdapter extends BaseAdapter {
    ...
    //constructor
    public SeaFoodProductAdapter(List<Product> list, LayoutInflater inflater, boolean showQuantity, boolean showPrice) {
        mProductList = list;
        mInflater = inflater;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int product_quantity = SeaFoodShoppingCartHelper.getProductQuantity(curProduct);
        double product_price = curProduct.price;

        //if the user hasn't added any products from the sea food category to their shopping cart, set the quantity to one (for calculation purposes)
        //if(SeaFoodShoppingCartHelper.getProductQuantity(curProduct) == 0)
        //{
            //product_quantity = 1;
        //}
        ...
        if (mShowQuantity) {
            item.productQuantity.setText("Quantity: "
                    + SeaFoodShoppingCartHelper.getProductQuantity(curProduct));
        } else {
            // Hid the view
